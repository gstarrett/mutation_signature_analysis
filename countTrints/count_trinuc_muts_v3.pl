...
my $name = "$variants.ann";
my $trinucs = "#chr\tpos\t5'tetranuc\t3'tetranuc\ttrinuc\tmut\ttrinuc_mut\tstrand\n";
...
while (my $line = <VAR>) {
	...
	if ($ref =~ /^[TC]$/) {
		...
		$trinucs .= "$chr\t$pos\t$tetraseq5\t$tetraseq3\t$seq\t$ref>$alt\t$fullstring\t1\t$sample\n";
		...
		$trinucs .= "$chr\t$pos\t$tetraseq5\t$tetraseq3\t$seq\t$compDict{$ref}>$compDict{$alt}\t$fullstring\t-1\t$sample\n";
	
	} else { next }
}
open (OUT, "> $name.trinucs_count.txt");

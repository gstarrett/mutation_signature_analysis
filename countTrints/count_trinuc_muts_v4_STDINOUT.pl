...
while (my $line = <VAR>) {
	...
	if ($ref =~ /^[TC]$/) {
		...
		$trinucs .= "$chr\t$pos\t$tetraseq5\t$tetraseq3\t$seq\t$ref>$alt\t$fullstring\t1\t$A3cxt\t$CGcxt\n";
		...
		$trinucs .= "$chr\t$pos\t$tetraseq5\t$tetraseq3\t$seq\t$compDict{$ref}>$compDict{$alt}\t$fullstring\t-1\t$A3cxt\t$CGcxt\n";

...
my (%earlyHash,%midHash,%lateHash,@earlys,@lates,@mids);

# split files by geno
for my $key (sort keys %genoHash) {
	open($earlyHash{$key}, "> $pre.early.$key");
	push(@earlys,"$pre.early.$key");
	open($midHash{$key}, "> $pre.mid.$key");
	push(@mids,"$pre.mid.$key");
	open($lateHash{$key}, "> $pre.late.$key");
	push(@lates,"$pre.late.$key");
}
...
while (my $line = <SOM>) {
	...
	if ($type eq "BRCA") {
		...
		$af = $f[43]/($f[42]);
		$cov = ($f[42]+$f[43]);
	} elsif ($type eq "LUSC" || $type eq "PRAD" || $type eq "HNSC" || $type eq "THCA" || $type eq "GBM" || $type eq "READ" || $type eq "COAD" || $type eq "KIRC" || $type eq "STAD" || $type eq "OV") {
		$af = $f[79]/($f[80] + $f[79]);
		$cov = ($f[80]+$f[79]);
	} elsif ($type eq "LGG") {
		$af = $f[36]/($f[37] + $f[36]);
	...
	if ($cn > 2.2) {
		...
		if ($cutoff[0] < 0.9/$cn) {
			print LATE $line;
			next unless exists $invGenoHash{$sample};
			my $lookup = $invGenoHash{$sample};
			print {$lateHash{$lookup}} $line if exists $lateHash{$lookup};
		} elsif ($cutoff[1] > 1.8/$cn) {
			print EARLY $line;
			next unless exists $invGenoHash{$sample};
			my $lookup = $invGenoHash{$sample};
			print {$midHash{$lookup}} $line if exists $midHash{$lookup};
		} elsif ($cutoff[1] >= 0.9/$cn) {
			print MID $line;
			next unless exists $invGenoHash{$sample};
			my $lookup = $invGenoHash{$sample};
			print {$earlyHash{$lookup}} $line if exists $earlyHash{$lookup};
		}
	} elsif ($cn <= 2.2 && $cn >= 1.8) {
...
cancer.a3hI.late <- read.table(\"$lates[0]\",sep=\"\t\")
cancer.noa3hI.late <- read.table(\"$lates[1]\",sep=\"\t\")
cancer.a3hI.early <- read.table(\"$earlys[0]\",sep=\"\t\")
cancer.noa3hI.early <- read.table(\"$earlys[1]\",sep=\"\t\")
cancer.a3hI.mid <- read.table(\"$mids[0]\",sep=\"\t\")
cancer.noa3hI.mid <- read.table(\"$mids[1]\",sep=\"\t\")
...
write.table(\"$pre.txt\",sep=\"\t\")
";

my $time = time();
open(ROUT, "> script$time.R");
print ROUT $script;

system("Rscript script$time.R");
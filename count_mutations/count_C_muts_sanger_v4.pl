...
my $bwamem = "/Users/starrettgj/soft/bwa-0.7.12/bwa mem";
...
if ($infile =~ /fasta$|fa$/) {
	...
	my @reflines = <REF>;
	my $refline;
	for (my $i = 1; $i < scalar @reflines; $i++) {
		chomp($reflines[$i]);
		$refline .= $reflines[$i];
	}
	my $qual = "+\n" . "I" x length($refline);
	open(QUAL, "> ref_tmp.fq");
	print QUAL join("\n", "\@reference_sequence",$refline,$qual,"");
...
my @ref = split('', $refseq);
my @qscore;
for (my $idx=0; $idx < scalar @seqs; $idx++) {
	chomp($seqs[$idx]);
	next unless defined $seqs[$idx];
	...
	my %readBaseHash;
	my $prevContext = "";
	for (my $sidx=0; $sidx < scalar @bases; $sidx++) {
	...
	for my $key (keys %counthash) {
		push(@prefilter, "$idx\t$key\t$counthash{$key}\t" . scalar @bases);
	}
}
...
sub calculate {
	...
	my $stdev = sqrt($sumDev/(scalar keys %$hash));
	for my $key (keys %$hash) {

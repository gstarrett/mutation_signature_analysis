...
my $name = "$variants";
my $trinucs = "#chr\tpos\t5'tetranuc\t3'tetranuc\ttrinuc\tmut\ttrinuc_mut\tstrand\tflank41bp\tCcount\tTCcount\tTCAcount\tTCTcount\tYTCAcount\tRTCAcount\tsample\tAF\n";
...
while (my $line = <VAR>) {
	...
	unless ($type eq "maf") {
		...
		$pos = shift @fields;
		$pos++;
		my $end = shift @fields if $type eq "bed";
	...
	if ($ref =~ /^[TC]$/) {
		...
		$trinucs .= "$chr\t$pos\t$tetraseq5\t$tetraseq3\t$seq\t$ref>$alt\t$fullstring\t1\t$flank40\t$CGcxt\t$TCNcxt\t$A3cxt1\t$A3cxt2\t$YtetraCxt\t$RtetraCxt\t$sample\t$AF\n";
		...
		$trinucs .= "$chr\t$pos\t$tetraseq5\t$tetraseq3\t$seq\t$compDict{$ref}>$compDict{$alt}\t$fullstring\t-1\t$flank40\t$CGcxt\t$TCNcxt\t$A3cxt1\t$A3cxt2\t$YtetraCxt\t$RtetraCxt\t$sample\t$AF\n";
